...
/**
 * Deletes a notification. For student, notification is deleted if provided notification id belongs
 * to the user. In case, it does not, silently ignore the operation. For teachers, a check is made
 * to make sure that class id for the specified notification belongs to user as teacher or
 * co-teacher. In case, notification does not exists, or user is not authorized, we silently ignore
 * the operation. NOTE: In case of non existence, or no authorization, this implementation silently
 * ignores operation. NOTE: Only non actionable notifications can be deleted/reset using this
 * service
 *
 * @author ashish.
 */
...
class ResetNotificationsService {
  ...
  void reset(ResetNotificationsCommand command) {
    ...
    if (notificationCanBeReset()) {
      ...
      LOGGER.warn("User '{}' Tried to reset actionable/deleted notification: '{}'",
          command.getUserId(), command.getNotificationId());
    }
  }
  ...
  private boolean notificationCanBeReset() {
    ...
    if (actionable == null) {
      // DB field is not null. If we get it from DB then it must be true/false. Else it does not
      // exist
      LOGGER.warn("User: '{}' tried to delete already deleted notification: '{}'.",
          command.getUserId(), command.getNotificationId());
      return false;
    }
    return !actionable;
  ...
  private void resetTeacherNotification() {
    ...
    if (classId != null) {
      ...
      if (getDao().checkUserIsAuthorizedAsTeacherForClass(classId, command.getUserId())) {
        ...
        LOGGER.warn("The user '{}' is not authorized to delete notification: '{}'.",
            command.getUserId(), command.getNotificationId());
      }
    } else {

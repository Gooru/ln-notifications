...
class FetchStudentNotificationsService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FetchStudentNotificationsService.class);
    ...
    private FetchStudentNotificationsResponse fetchStudentNotificationForSingleClass() {
        if (!getDao().checkUserIsStudentForClass(command.getClassId(), command.getUserId())) {
            LOGGER.warn("User '{}' is not student for class '{}' or class does not exists", command.getUserId(),
                command.getClassId());

...
interface FetchStudentNotificationsDao {
    ...
    @SqlQuery(
        "select id, ctx_class_id, ctx_class_code, ctx_course_id, ctx_unit_id, ctx_lesson_id, ctx_collection_id,  " +
            ...
            "updated_at, milestone_id, ctx_source from student_notifications where ctx_class_id = :classId and " +
            "ctx_user_id = :userId and  to_timestamp(:boundary) > updated_at order by updated_at desc limit :limit")

...
class FetchTeacherNotificationsService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FetchTeacherNotificationsService.class);
    ...
    private FetchTeacherNotificationsResponse fetchTeacherNotificationForSingleClass() {
        if (!getDao().checkUserIsAuthorizedAsTeacherForClass(command.getClassId(), command.getUserId())) {
            LOGGER.warn("User '{}' is not authorized for class '{}' or class does not exists", command.getUserId(),
                command.getClassId());

...
interface FetchTeacherNotificationsDao {
  ...
  @SqlQuery("select id, ctx_class_id, ctx_class_code, ctx_course_id, ctx_unit_id, ctx_lesson_id, ctx_collection_id,  "
      + "current_item_id, current_item_type, current_item_title, notification_type, ctx_path_id, ctx_path_type,  "
      + "occurrence, updated_at, milestone_id, ctx_source  from teacher_notifications where ctx_class_id = "
      + ":classId and  to_timestamp(:boundary) > updated_at order by updated_at desc limit :limit")
  ...
  @SqlQuery("select id, ctx_class_id, ctx_class_code, ctx_course_id, ctx_unit_id, ctx_lesson_id, ctx_collection_id,  "
      + "current_item_id, current_item_type, current_item_title, notification_type, ctx_path_id, ctx_path_type,  "
      + "occurrence, updated_at,  milestone_id, ctx_source  from teacher_notifications where ctx_class_id = any"
      + "(select id from class where  (creator_id = :userId or collaborator ?? :userId::text) and is_deleted = "
      + "false) and  to_timestamp(:boundary) > updated_at order by updated_at desc limit :limit")

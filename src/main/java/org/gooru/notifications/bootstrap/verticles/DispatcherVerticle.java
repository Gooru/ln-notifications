...
public class DispatcherVerticle extends AbstractVerticle {
  ...
  public void start(Future<Void> startFuture) {
    ...
    eb.localConsumer(Constants.EventBus.MBEP_NOTIFICATIONS_DISPATCHER, this::processMessage)
  ...
  private static void futureResultHandler(Message<JsonObject> message,
      Future<MessageResponse> future, boolean replyNeeded) {
      ...
      if (event.succeeded() && replyNeeded) {
        ...
        if (event.cause() instanceof HttpResponseWrapperException) {
          ...
          MessageResponseWrapperException exception =
              (MessageResponseWrapperException) event.cause();

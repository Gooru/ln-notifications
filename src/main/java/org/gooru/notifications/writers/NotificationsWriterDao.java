...
interface NotificationsWriterDao {
  ...
  @SqlUpdate("insert into teacher_notifications (ctx_class_id, ctx_class_code, ctx_course_id, ctx_unit_id, ctx_lesson_id, "
  ...
  void updateExistingNotificationForNewUser(@Bind("id") Long id, @Bind("occurrence") int occurrence,
      @Bind("usersArray") PGArray<String> usersArray);

  @Mapper(TeacherNotificationsModelMapper.class)
  @SqlQuery("select id, ctx_class_id, ctx_course_id, ctx_unit_id, ctx_lesson_id, ctx_collection_id, current_item_id, "
      + "current_item_type, notification_type, ctx_path_id, ctx_path_type, occurrence, users, ctx_source, "
      + "milestone_id from teacher_notifications where ctx_class_id = :classId and ctx_course_id = :courseId and "
      + "ctx_unit_id = :unitId and ctx_lesson_id = :lessonId and ctx_collection_id is null and current_item_id = "
  ...
  TeacherNotificationsModel findTeacherNotificationForContextWithoutCollectionAndPath(
      @BindBean NotificationsConsumerCommandBean bean);
  ...
  TeacherNotificationsModel findTeacherNotificationForContextWithoutCollection(
      @BindBean NotificationsConsumerCommandBean bean);
  ...
  @SqlQuery("select id, ctx_class_id, ctx_course_id, ctx_unit_id, ctx_lesson_id, ctx_collection_id, current_item_id, "
      + "current_item_type, notification_type, ctx_path_id, ctx_path_type, occurrence, users, ctx_source, "
      + "milestone_id from teacher_notifications where ctx_class_id = :classId and ctx_course_id = :courseId and "
      + "ctx_unit_id = :unitId and ctx_lesson_id = :lessonId and ctx_collection_id = :collectionId and "
  ...
  TeacherNotificationsModel findTeacherNotificationForContextWithoutPath(
      @BindBean NotificationsConsumerCommandBean bean);
  ...
  TeacherNotificationsModel findTeacherNotificationForContext(
      @BindBean NotificationsConsumerCommandBean bean);

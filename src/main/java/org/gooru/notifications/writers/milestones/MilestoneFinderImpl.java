...
class MilestoneFinderImpl implements MilestoneFinder {
    ...
    /*
        Iff,
            class is not deleted and
            class has course which is premium and
            class has current grade set
            then
                fetch fw from current grade
                fetch milestone with CUL and fw_code
                return fw_code
        else
            no milestone id present
     */
    ...
    public String findMilestone() {
        ...
        if (destination != null) {
            return fetchDao().fetchMilestoneId(context.getCourseId(), context.getUnitId(), context.getLessonId(),
                destination);
        }
        return null;
    }

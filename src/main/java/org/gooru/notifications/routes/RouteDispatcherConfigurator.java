...
public class RouteDispatcherConfigurator implements RouteConfigurator {
    ...
    public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
        ...
        router.get(Constants.Route.API_NOTIFICATION_FETCH_TEACHER).handler(this::fetchTeacherNotifications);
        router.get(Constants.Route.API_NOTIFICATION_FETCH_STUDENT).handler(this::fetchStudentSuggestions);
        router.delete(Constants.Route.API_NOTIFICATION_STUDENT_RESET).handler(this::deleteNotificationStudent);
        router.delete(Constants.Route.API_NOTIFICATION_TEACHER_RESET).handler(this::deleteNotificationTeacher);
    }
    ...
    private void deleteNotification(RoutingContext routingContext, String role) {
        DeliveryOptions options = DeliveryOptionsBuilder.buildWithApiVersion(routingContext).setSendTimeout(mbusTimeout)
                                      .addHeader(Constants.Message.MSG_OP,
                                          Constants.Message.MSG_OP_NOTIFICATIONS_RESET);
    ...
    private Long validateAndGetLong(String id) {
        if (id != null && !id.isEmpty()) {
            try {
                ...
                throw new HttpResponseWrapperException(HttpConstants.HttpStatus.BAD_REQUEST, "Invalid notification id");
            }
        }
        throw new HttpResponseWrapperException(HttpConstants.HttpStatus.BAD_REQUEST, "Invalid notification id");
    }
    ...
    private void fetchStudentSuggestions(RoutingContext routingContext) {
        DeliveryOptions options = DeliveryOptionsBuilder.buildWithApiVersion(routingContext).setSendTimeout(mbusTimeout)
                                      .addHeader(Constants.Message.MSG_OP,
                                          Constants.Message.MSG_OP_NOTIFICATIONS_STUDENT);
        eb.<JsonObject>send(Constants.EventBus.MBEP_NOTIFICATIONS_DISPATCHER,
    ...
    private void fetchTeacherNotifications(RoutingContext routingContext) {
        DeliveryOptions options = DeliveryOptionsBuilder.buildWithApiVersion(routingContext).setSendTimeout(mbusTimeout)
                                      .addHeader(Constants.Message.MSG_OP,
                                          Constants.Message.MSG_OP_NOTIFICATIONS_TEACHER);
        eb.<JsonObject>send(Constants.EventBus.MBEP_NOTIFICATIONS_DISPATCHER,
            RouteRequestUtility.getBodyForMessage(routingContext), options,
            reply -> RouteResponseUtility.responseHandler(routingContext, reply, LOGGER));
    }

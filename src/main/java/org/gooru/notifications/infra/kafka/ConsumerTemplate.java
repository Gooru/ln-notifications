...
/**
 * This is the base class on which all DEPs will be built. It encapsulates the infra code needed to
 * create and configure Kafka consumer, deploy it and assign a thread to it.
 *
...
public abstract class ConsumerTemplate<K, V> implements Runnable, Deployable {
  ...
  /**
   * Concrete classes should implement this method to setup the props correctly by calling
   * {@link #initialize}
   */
  public abstract void init();

  /**
   * The handler which should be used in case an exception is thrown from
   * {@link #processRecord(ConsumerRecord)}
   *
   * @param record
   * @param e
   */
  ...
  /**
   * The handler which will be invoked when the commit failed after processing the batch of records
   * received in poll
   *
   * @param e
   */
   ...
   * {@link ConsumerRecord} that is read on the topic. Note that in case of any exception that will
   * happen during processing, concrete classes should handle them and take actions like logging or
   * putting it to error topic. The default behavior in base class is to catch error and *commit*
   * the record as the loop needs to continue processing the next records.
   *
   * @param record
   */
  ...
  public void run() {
    ...
    try {
      ...
      Long pollInterval = Long.parseLong(
          kafkaConsumerConfig.getProperties().getProperty("max.poll.interval.ms", "1000"));
